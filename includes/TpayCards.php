...
//obliczanie koszyka na nowo jesli jest doplata za tpay.com
    ...
    public function gateway_communication()
    {
        if (filter_input(INPUT_GET, static::ORDER_ID) && (filter_input(INPUT_GET, static::CARDDATA))) {
            ...
            if (filter_input(INPUT_GET, static::CLIENTNAME)) {
                $transactionData['nazwisko'] = filter_input(INPUT_GET, static::CLIENTNAME);
            }
            if (filter_input(INPUT_GET, static::CLIENTEMAIL)) {
                $transactionData['email'] = filter_input(INPUT_GET, static::CLIENTEMAIL);
            }
    ...
    {
        if (empty($_POST[static::CARDDATA]) || empty($_POST[static::CLIENTEMAIL])
            || empty($_POST[static::CLIENTNAME])
        ) {
            return false;
        }
        global $woocommerce;
        ...
        return array(
            ...
            static::REDIRECT => add_query_arg(array(
                ...
                static::CLIENTNAME  => filter_input(INPUT_POST, static::CLIENTNAME),
                static::CLIENTEMAIL => filter_input(INPUT_POST, static::CLIENTEMAIL),
